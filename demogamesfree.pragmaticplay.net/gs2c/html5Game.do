...
<script type="text/javascript" src="common/js/html5-script-external.js"></script>
    <script type="text/javascript">
        (function(){
            var rawConfig = '{"accountType":"R","currency":"USD","currencyOriginal":"USD","datapath":"https://demogamesfree.pragmaticplay.net/gs2c/common/v1/games-html5/games/vs/vs40cosmiccash/","gameService":"https://demogamesfree.pragmaticplay.net/gs2c/ge/v4/gameService","lang":"en","vendor":"T","showRealCash":"1","clock":"0","mgckey":"stylename@generic~SESSION@8527f8b8-9199-496b-b814-4e5cd7b358ce","styleName":"generic","extend_events":"1","REGULATION":"https://demogamesfree.pragmaticplay.net/gs2c/regulation/process.do?symbol\u003dvs40cosmiccash","statisticsURL":"https://demogamesfree.pragmaticplay.net/gs2c/stats.do","jurisdiction":"99","langSelectorEnabled":"0","cid":"11111","demoMode":"1","manualPayout":["MRC","TMC","TMFS","MRFS"],"LOGOUT":"/gs2c/logout.do","RELOAD_BALANCE":"/gs2c/reloadBalance.do","RELOAD_JACKPOT":"/gs2c/jackpot/reload.do","SETTINGS":"/gs2c/saveSettings.do","CLOSE_GAME":"/gs2c/closeGame.do?symbol\u003dvs40cosmiccash","region":"Other","sessionKeyV2":["lJStAFlJBe7w2K79eXuCXZkRNhTHNBLPCgn1AFF0wD1vMTE6GZvxRpfcZJrd9qO7wJN8Lc+UrRWRXLPrhEVqVFra57YDEcdbN3rkhjW4j9RJu2SKJhdzRTqeH/iMH4X0Lv01+LoZUm7intu2TRTeWXq1NxI8ElVR9wcFTP4YNcY\u003d","O93cFdElCCG7nRHL+wxvZKw1rhP9K7zq4S97BtW8J7nixp45gbfQTxbwR4csMID105ZWh4rZzKRJv0E02LRlv8+m7JK1DBNc2+Iri7/tGOW+Mh1khCuSz/l1t1AIhYZtehBuOV6eYkuEvJcd5rJaOqKh4jddkGE+isfHR5r7AMY\u003d"],"mid":"3893897757278627610"}';
            var base = window.location.origin + "/demogamesfree.pragmaticplay.net";
            var gameConfig = rawConfig.replace(/https:\/\/demogamesfree\.pragmaticplay\.net/g, base);
            Html5GameManager.init({
                ...
                mgckey: "stylename@generic~SESSION@8527f8b8-9199-496b-b814-4e5cd7b358ce",
                jurisdictionMsg: "",
                extendSessionUrl: "",
            ...
            var patchInterval = setInterval(function () {
                ...
                if (window.ServerInterface && ServerInterface.prototype.ParseConfigFile) {
                    ...
                    ServerInterface.prototype.ParseConfigFile = function () {
                        if (!this.configFile || typeof this.configFile.text !== "string") {
                            // Minimal defaults for local offline run
                            ServerOptions.styleName = "generic";
                            ServerOptions.gameSymbol = "vs40cosmiccash";
                            ServerOptions.language = "en";
                            ServerOptions.isOnline = false;
                            ServerOptions.isReplay = false;
                ...
                // Patch CAT_Infector.Infect to be defensive when hostCat/events/actions are missing
                if (window.CAT_Infector && CAT_Infector.prototype.Infect) {
                    CAT_Infector.prototype.Infect = function (evtInfection, hostCat) {
                        if (
                            !hostCat ||
                            !Array.isArray(hostCat.events) ||
                            !evtInfection ||
                            !evtInfection.parasiteLink ||
                            !evtInfection.parasiteLink.cat ||
                            !evtInfection.parasiteLink.cat.GetEventByID
                        ) {
                            return;
                        }
                        if (evtInfection.parasiteLink.cat.GetEventByID(evtInfection.parasiteLink.id) == null) {
                            return;
                        }
                        for (var i = 0; i < hostCat.events.length; i++) {
                            var ev = hostCat.events[i];
                            if (!ev || typeof ev.id === "undefined") continue;
                            if (!Array.isArray(ev.actions)) ev.actions = [];
                            if (ev.id === evtInfection.hostCATEventID) {
                                ev.actions.push(evtInfection.AsCatAction());
                ...
                // Patch CurveTwoValuesAnimator.Awake to handle missing curves
                if (window.CurveTwoValuesAnimator && CurveTwoValuesAnimator.prototype.Awake) {
                    CurveTwoValuesAnimator.prototype.Awake = function () {
                        if (
                            !this.curve_1 ||
                            !this.curve_1.keys ||
                            this.curve_1.keys.length === 0 ||
                            !this.curve_2 ||
                            !this.curve_2.keys ||
                            this.curve_2.keys.length === 0
                        ) {
                ...
                // Patch WidgetColorAnimator.Start to handle missing animationCurve / keys
                if (window.WidgetColorAnimator && WidgetColorAnimator.prototype.Start) {
                    var originalWidgetStart = WidgetColorAnimator.prototype.Start;
                    WidgetColorAnimator.prototype.Start = function () {
                        this.direction = this.playReverse ? -1 : 1;
                        if (
                            !this.animationCurve ||
                            !this.animationCurve.keys ||
                            this.animationCurve.keys.length === 0
                        ) {
                            this.firstCurveKey = { time: 0 };
                            this.lastCurveKey = { time: 0 };
                            this.totalCurveTime = 0;
                            // still allow Update to run using animationTime only
                            return;
                        }
                        this.firstCurveKey = this.animationCurve.keys[0];
                        this.lastCurveKey =
                            this.animationCurve.keys[this.animationCurve.keys.length - 1];
                        this.totalCurveTime = this.lastCurveKey.time - this.firstCurveKey.time;
                    };
                }

                // Patch ParticlesEnabler to tolerate missing ParticleSystem components
                if (window.ParticlesEnabler && ParticlesEnabler.prototype.Awake) {
                    ParticlesEnabler.prototype.Awake = function () {
                        if (!this.particles) {
                            var list = this.gameObject.GetComponentsInChildren(ParticleSystem, true);
                            this.particles = list && list.length > 0 ? list[0] : null;
                        }
                        if (!this.particles) {
                            this.pEnabled = false;
                        ...
                        if (!this.pEnabled && this.particles.gameObject) {
                            this.particles.gameObject.SetActive(false);
                        }
                    };

                    var origEnableEmission = ParticlesEnabler.prototype.EnableEmission;
                ...
                // Patch SpriteSizeAnimator.Awake to handle missing animationCurve / keys
                if (window.SpriteSizeAnimator && SpriteSizeAnimator.prototype.Awake) {
                ...
                // Patch CurveValueAnimator.Awake to handle missing curve / keys
                if (window.CurveValueAnimator && CurveValueAnimator.prototype.Awake) {
                    CurveValueAnimator.prototype.Awake = function () {
                        if (!this.curve || !this.curve.keys || this.curve.keys.length === 0) {
                            this.lastKeyTime = 0;
                            return;
                        }
                ...
                // Patch MenuWindowControllerMobile.Awake to tolerate missing settings entries
                if (window.MenuWindowControllerMobile && MenuWindowControllerMobile.prototype.Awake) {
                    var origMWCMAwake = MenuWindowControllerMobile.prototype.Awake;
                    MenuWindowControllerMobile.prototype.Awake = function () {
                        try {
                            if (
                                ...
                                !this.settings[0] ||
                                !this.settings[1] ||
                                !this.settings[0].transform ||
                                !this.settings[1].transform
                            ) {
                                // Settings are malformed; skip logo position patch to avoid crashes
                                return;
                            }
                        } catch (e) {
                            // On unexpected errors with settings, just skip patch
                            return;
                        }
                        return origMWCMAwake.apply(this, arguments);
                    };
                }

                // Patch MenuController.Awake to handle missing order array
                if (window.MenuController && MenuController.prototype.Awake) {
                    var origMCAwake = MenuController.prototype.Awake;
                    MenuController.prototype.Awake = function () {
                        // If order is missing, skip menu setup to avoid crashes on local clone
                ...
                // Patch AnimationCurve.Evaluate to handle missing keyframes gracefully
                if (window.AnimationCurve && AnimationCurve.prototype.Evaluate) {
                    var origEvaluate = AnimationCurve.prototype.Evaluate;
                    AnimationCurve.prototype.Evaluate = function (animTime) {
                        if (!this.keys || this.keys.length === 0) {
                            return 0;
                        }
                        return origEvaluate.call(this, animTime);
                    };
                }

                // Patch logo_info.js Chinese sound patch to tolerate missing asiaContents
                if (window.PatchAGCC) {
                    var origPatchAGCC = window.PatchAGCC;
                    window.PatchAGCC = function () {
                        try {
                            if (
                                window.globalRuntime &&
                                globalRuntime.sceneRoots &&
                                globalRuntime.sceneRoots.length > 1 &&
                                window.PromotionContentSwitcher
                            ) {
                                var pcs = globalRuntime.sceneRoots[1].GetComponentsInChildren(
                                    PromotionContentSwitcher,
                                    true
                                );
                                for (var s = 0; s < pcs.length; s++) {
                                    var pc = pcs[s];
                                    if (!pc || !pc.asiaContents || !pc.europeContents) continue;
                                    for (var a = 0; a < pc.asiaContents.length; a++) {
                                        var asp = pc.asiaContents[a] && pc.asiaContents[a].GetComponent
                                            ? pc.asiaContents[a].GetComponent(SoundPlayer)
                                            : null;
                                        if (asp && a < pc.europeContents.length) {
                                            var esp = pc.europeContents[a] && pc.europeContents[a].GetComponent
                                                ? pc.europeContents[a].GetComponent(SoundPlayer)
                                                : null;
                                            if (esp) asp.audioClip = esp.audioClip;
                                        }
                                    }
                                }
                                // mark fixed to stop retries
                                if (typeof fixed !== "undefined") fixed = true;
                                return;
                            }
                        } catch (e) {
                            // ignore and fall back to original implementation if needed
                        }
                        // Fallback to original if our safe path didn't run
                        return origPatchAGCC();
                    };
                }
            }, 50);
        })();
